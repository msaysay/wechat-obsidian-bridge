/**
 * Claude Code Agent 桥：把微信消息交给本地 claude.exe 处理（cwd=Obsidian 库）。
 * v0.2 改用 stream-json：Agent 说话/用工具的过程实时回调 onEvent，
 * 由上层转成微信的流式消息(GENERATING态)和工具状态提示。
...
export class AgentBridge {
   ...
   * 跑一轮 Agent，返回最终回复文本。
   * onEvent 可选，流式事件：
   *   {kind:"text", text}          Agent 输出了一段话
   *   {kind:"tool", name, detail}  Agent 开始用某个工具
   * 会话失效时自动降级为新会话重试一次。
   */
